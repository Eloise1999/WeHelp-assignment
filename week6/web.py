...
def signin():
    ...
    command = "select * from member where username='" + username + "' and password='" + password+ "'"
    print(command)
...
def signup():
    ...
    command = "select * from member where username='" + username + "'"
    print(command)
    # MySQL 指令：執行
    cursor.execute(command)
    select_result = cursor.fetchall()
    ...
    if len(select_result) != 0:
        ...
        command = "insert into member(name, username, password) values ('" + name + "','" + username + "','" + password + "')"
        print(command)
        cursor.execute(command)
